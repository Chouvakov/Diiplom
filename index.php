...
<body>
    ...
    <div class="header">
        ...
        <div class="header-section">
            <div class="header-item button"><a href="registration.html">
                    Регистрация</a></div>
            <div class="header-item button"><a href="autorization.html">
                    Вход</a></div>
        </div>
    </div>
    <?php
    if ($_REQUEST['action'] === 'add_topic' && $_COOKIE['user_id']) {
        ...
        $resultat = mysqli_query($mysql, "INSERT INTO `posts` (`name`,`countmessage`,`user_id`,`datecreate`) VALUES ('{$_REQUEST['topic']}','0','{$_COOKIE['user_id']}'," . time() . ")");
     ...
     <table border="3" class="tabler">
        ...
        <?php foreach ($postlist as $topic): ?>
            <tr>
                ...
                <td><a href="#"><?= date('d-m-Y H:i:s', $topic['DateReply']) ?></a></td>
            </tr>
        <?php endforeach ?>
    </table>
